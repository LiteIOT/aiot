强化学习如果要在自动控制领域落地，免不了要和真实的硬件设备打交道。当前的强化学习框架主要是基于Python开发，部署Python设备硬件至少能跑操作系统，实际项目中如果要训练一个能用的模型，一般需要基于硬件环境建模，开发一个模拟器，先在模拟器上开发，然后再部署到实际平台上测试，一次肯定不会成功的，还需要修改模型，修改参数，重新训练，重新部署，如此往复，直到最后能得到满足实际需求的模型和参数。
